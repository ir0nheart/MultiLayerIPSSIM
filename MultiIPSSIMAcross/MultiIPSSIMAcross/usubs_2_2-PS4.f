...
C                                                                        BCTIME........1100
     ...
     1   K10,K11,K12,K13,KNODEL,KGRADIENT                                                 BCTIME........2200
...
C     UBC(IP) =  ((          ))
C
C      IF (TMIN .LT. 13.D0)  THEN
C      PBC(IP) = ((-0.0086*(TMIN**3))+(0.2439D0*(TMIN**2))
C     1        -(2.0398D0*TMIN)+45.499D0)*98.07D0
C      UBC(IP) = 0.003248D0
C      END IF
C      IF (TMIN .GT. 13.D0)  THEN
C      PBC(IP) = 2501.55D0
C      UBC(IP) = 0.0D0
C      END IF
                                                                         BCTIME.......12700
...
C SUBROUTINE BUBSAT FOR BUBBLE SATURATION
      SUBROUTINE BUBSAT (SWB,RELKB,PRES,CNUB,RELKT,SWT,SW,RELK)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      COMMON /CONST/ PSTAR,GCONST,TEMP,SMWH
     ...
     2   ISTORE,NOUMAT,IUNSAT,KTYPE
      ...
      SAVE N
       ...
       RELKB=SWB**N
ccc       RELKT=1.0D0                                                     ! alden 110111 commented out
       SWT=SWB*SW
       RELKT=RELKB*RELK                                                   ! alden 110111 uncommented
      RETURN
      END
